...
public class ErraiEntityPageBuilder extends AbstractEntityArtifactProcessor {
	...
	private static final String BASE_ENTITY_VIEW_FTL = "ui/page-components/base-entity-view.ftl";
	...
	private void doBuildPages(BuildConfiguration buildConfiguration, Entity entity) throws IDEException {
		if(entity.isHasUI()){
			...
			BuilderUtils.generateArtifact(buildConfiguration, entity, 
				BASE_ENTITY_VIEW_FTL, baseEntityView, targetDir.concat(getEntityPath(entity)));
	...
	private void buildListView(BuildConfiguration buildConfiguration, Entity entity) throws IDEException {
		...
		BuilderUtils.generateArtifact(buildConfiguration, entity, 
			entity.getListPageTemplate(), listPageHTML, targetDir.concat(getEntityPath(entity)));
		
		BuilderUtils.generateArtifact(buildConfiguration, entity, 
			entity.getListViewTemplate(), listPageView, targetDir.concat(getEntityPath(entity)));
		
		BuilderUtils.generateArtifact(buildConfiguration, entity, 
			entity.getListItemTemplate(), listPageItemView, targetDir.concat(getEntityPath(entity)));
	}
	...
	private void buildCreateView(BuildConfiguration buildConfiguration, Entity entity) throws IDEException {
		...
		BuilderUtils.generateArtifact(buildConfiguration, entity, 
				entity.getCreatePageTemplate(), createPageHTML, targetDir.concat(getEntityPath(entity)));
		
		BuilderUtils.generateArtifact(buildConfiguration, entity, 
				entity.getCreateViewTemplate(), createPageView, targetDir.concat(getEntityPath(entity)));
		
		BuilderUtils.generateArtifact(buildConfiguration, entity, 
				entity.getEditorTemplate(), editorComponent, targetDir.concat(getEntityPath(entity)));
	 ...
	 * @return
	 */
	protected String getEntityPath(Entity entity) {
		return IDEUtils.getModulePath(entity).concat(entity.getName().toLowerCase().concat("/"));
	}
	...
	private void buildSingleView(BuildConfiguration buildConfiguration, Entity entity) throws IDEException {
		...
		BuilderUtils.generateArtifact(buildConfiguration, entity, 
			entity.getSinglePageTemplate(), singlePageHTML, targetDir.concat(getEntityPath(entity)));
		
		BuilderUtils.generateArtifact(buildConfiguration, entity, 
			entity.getSingleViewTemplate(), singlePageView, targetDir.concat(getEntityPath(entity)));
		
		BuilderUtils.generateArtifact(buildConfiguration, entity, 
			entity.getSingleDisplayTemplate(), displayComponent, targetDir.concat(getEntityPath(entity)));
	}
}
